...
class AwsInvokeLocal {
      ...
      (
        ...
        if (!exists) {
          ...
          if (!cacheExists) {
            ...
            await fse.ensureDir(path.join(layerContentsCachePath))
  ...
  async buildDockerImage(layerPaths) {
    ...
    await Promise.all([
      fse.ensureDir(path.join('.serverless', 'invokeLocal', runtime)),
      fse.ensureDir(path.join(cachePath, 'dockerfiles', runtime)),
    ])
